...
Spotlight calculates the number of times the file group could grow before hitting the defined limits. The Databases - Dynamic Data File Group Growths Remaining alarm is raised when the disk space is insufficient for files to keep growing into or the maximum file size (if it is set) is being approached.

This alarm is ignored if **Auto Grow=No** for all files in the file group and the growths remaining is set to null.
...
This alarm can be configured to ignore certain values.
