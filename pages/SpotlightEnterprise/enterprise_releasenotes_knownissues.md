...
**Manage access to Spotlight connections.** The Spotlight Client ignores an error returned by Diagnostic Server when attempting to create a connection that already exists and is in a blacklist. <br><br> *Workaround:* Do not attempt to create the connection. | SOSSE-8189
...
**Manage access to Spotlight connections.** For efficiency reasons the Diagnostic Server only checks Active Directory group membership periodically. <br><br> In the following scenarios a user may still have access to connections affected by a blacklist <br><br> 1. An Active Directory group is removed from or added to a Diagnostic Server group. <br> 2. The user is removed from or added to a Diagnostic Server group. <br> 3. The user is removed from or added to an Active Directory group that is a member of a Diagnostic Server group. <br><br> *Workaround:* Wait 10 minutes or restart the Diagnostic Server. After that the change should take effect. | SOSSE-8222
**Manage access to Spotlight connections.** A Windows user with Non-Unicode characters in any of the three groups Spotlight Diagnostic Administrators, Spotlight Diagnostic Users and Spotlight Diagnostic Read-Only Users is not being recognized and displayed properly. However the issue only happens in Manage access to Spotlight connections and it does not affect any other Spotlight features. <br><br> *Workaround:* Do not use Non-Unicode characters for the users who require access to Spotlight. | SOSSE-8203
...
**Manage access to Spotlight connections.** If a user is granted access to a SQL Analysis connection but is denied access to the associated Windows connection Spotlight still shows the Windows metrics on the overview page of the SQL Analysis connection. | SOSSE-8199     
