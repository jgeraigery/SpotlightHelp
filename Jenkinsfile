...
timestamps
{
    nodeWithProperWorkspace('slavetemplate', 'SE')
    {
        ...
        def PARENTBRANCH=readFile(encoding: 'UTF-8', file: "${CurrentDir}/Spotlight/build/parentbranch.txt").trim()
        def DSBRANCH
        def CLIENTBRANCH=env.BRANCH_NAME
        ...
        dir("${CurrentDir}/OnlineHelp")
        {
            def buildhelp = load("${CurrentDir}/OnlineHelp/_build/BuildHelp.groovy")
            buildhelp.BuildHelp()
        }
